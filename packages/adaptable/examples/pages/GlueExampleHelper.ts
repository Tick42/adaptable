...
export class GlueExampleHelper {
  examplesHelper: ExamplesHelper = new ExamplesHelper();

  public async getTrades(count: number): Promise<IGlueDemoTrade[]> {
    const trades: IGlueDemoTrade[] = [];
    for (let i = 1; i <= count; i++) {
      trades.push(await this.createTrade(i));
    }
    return trades;
  }

  async createTrade(i: number, currency?: string): Promise<IGlueDemoTrade> {
    // contact
    let contact: any;
